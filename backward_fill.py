...
for year in range(2019, datetime.date.today().year):
    ...
    for month in range(1, 13):
        ...
        if reference_date_str >= datetime.date.today().isoformat():
            print(f"Reached current month {reference_date_str}, stopping.")
            break_condition = True
            break

        if reference_date_str <= "2019-09-01":
            print(f"Skipping month {reference_date_str} as it's before 2019-09-01.")
            continue
